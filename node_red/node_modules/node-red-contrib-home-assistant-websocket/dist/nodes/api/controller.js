...
class Api extends BaseNode {
    ...
    async onInput({ message, parsedMessage, send, done }) {
        ...
        const renderTemplate = generateRenderTemplate(message, this.node.context(), this.homeAssistant.getStates());
        let data;
        ...
        const results = await apiCall().catch((err) => {
            ...
            done(`API Error. ${err.message ? `Error Message: ${err.message}` : ''}`);
        });
        try {
