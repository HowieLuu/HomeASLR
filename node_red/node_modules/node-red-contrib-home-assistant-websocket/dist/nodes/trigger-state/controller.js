...
class TriggerState extends EventsHaNode {
    constructor({ node, config, RED, status }) {
        ...
        this.addEventClientListener(eventTopic, this.onEntityStateChanged.bind(this));
    ...
    onEntityStateChanged(evt) {
        ...
        if (!shouldIncludeEvent(eventMessage.entity_id, this.nodeConfig.entityid, this.nodeConfig.entityidfiltertype)) {
            return;
        }
        // Convert and save original state if needed
        this.castState(eventMessage.event.old_state, this.nodeConfig.state_type);
        this.castState(eventMessage.event.new_state, this.nodeConfig.state_type);
        try {
            eventMessage.event.new_state.timeSinceChangedMs =
                Date.now() -
                    new Date(eventMessage.event.new_state.last_changed).getTime();
            const constraintComparatorResults = this.getConstraintComparatorResults(this.nodeConfig.constraints, eventMessage);
            const statusText = `${eventMessage.event.new_state.state}${eventMessage.event_type === 'triggered' ? ' (triggered)' : ''}`;
    ...
    getConstraintComparatorResults(constraints, eventMessage) {
        ...
        for (const constraint of constraints) {
            ...
            const constraintTarget = this.getConstraintTargetData(constraint, eventMessage.event);
            const actualValue = selectn(constraint.propertyValue, constraintTarget.state);
    ...
    getCustomOutputsComparatorResults(outputs, eventMessage) {
        return outputs.reduce((acc, output) => {
            ...
            if (output.comparatorPropertyType !== 'always') {
                result.actualValue = selectn(output.comparatorPropertyValue, eventMessage.event);
    ...
    getConstraintTargetData(constraint, triggerEvent) {
        ...
        try {
            ...
            this.node.debug('Error during trigger:state comparator evaluation: ', e.stack);
            throw e;
        }
        return targetData;
    }
    getOutputMessage({ output, comparatorMatched, actualValue }, eventMessage) {
        ...
        if (output.messageType === 'custom' ||
            ...
            payload = renderTemplate(output.messageValue, eventMessage.event, this.node.context(), this.homeAssistant.getStates());
