...
class CurrentStateController extends InputOutputController_1.default {
    ...
    async onInput({ message, parsedMessage, send, done, }) {
        ...
        let isIfState = __classPrivateFieldGet(this, _CurrentStateController_comparatorService, "f").getComparatorResult(this.node.config.halt_if_compare, this.node.config.halt_if, entity.state, this.node.config.halt_if_type, {
            message,
            entity,
        });
        if (__classPrivateFieldGet(this, _CurrentStateController_instances, "m", _CurrentStateController_checkForDuration).call(this, isIfState)) {
            const forDurationMs = __classPrivateFieldGet(this, _CurrentStateController_instances, "m", _CurrentStateController_getForDurationMs).call(this, message);
            if (forDurationMs > 0) {
...
}, _CurrentStateController_getForDurationMs = function _CurrentStateController_getForDurationMs(message) {
    if (this.node.config.for === '')
        return 0;
    const value = this.typedInputService.getValue(this.node.config.for, this.node.config.forType, { message });
